...
class Tab5ViewController: TabItemViewController, UITableViewDelegate, UITableViewDataSource {

    let screenName = "benefits_controller"
    ...
    override func viewDidLoad() {
        ...
        self.news = AMGNews.shared.news.filter( { $0.newsTypeId == 17 } )
    ...
    @objc func actionOnGetNews() {
        AMGNews.shared.loadNews()
        news = AMGNews.shared.news.filter({ $0.newsTypeId == 17 })
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        YMReport(message: screenName, parameters: ["action":"open_benefit"])
        
        let newsVC = NewsViewController()
        newsVC.delegate2 = self
        newsVC.news = newsOne
