...
- (void)layoutSubviews
{
    ...
    _shapeLayer.frame = CGRectMake(5, 3, self.bounds.size.width - 8, self.bounds.size.height - 8);
    
    UIBezierPath *path = [UIBezierPath bezierPath];
    CGRect bounds = _shapeLayer.frame;
    
    [path moveToPoint:CGPointMake(5, 0)];
    [path addLineToPoint:CGPointMake(bounds.size.width, 0)];
    [path addLineToPoint:CGPointMake(bounds.size.width - 5, bounds.size.height)];
    [path addLineToPoint:CGPointMake(0, bounds.size.height)];
    
    if (!CGPathEqualToPath(_shapeLayer.path,[path CGPath])) {
...
- (void)configureAppearance
{
    ...
    if (!shouldHideShapeLayer) {
        ...
        //CGPathRef path = [UIBezierPath bezierPathWithRoundedRect:_shapeLayer.bounds cornerRadius:4].CGPath;
